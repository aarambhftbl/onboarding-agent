Project Title

Team Name - BroCoders

PS03 - Autonomous Developer Onboarding Agent

Hackathon
SYRUS 2026

Problem
Organizations lack autonomous systems to guide developers through onboarding.

Solution
AI-powered agent that:
personalizes onboarding
retrieves knowledge via RAG
tracks onboarding checklist
sends HR completion email

Prototype

Streamlit App:

https://app-agent-vvc6jo2t9fgppxnw9ghozj.streamlit.app/

Tech Stack
Python
Streamlit
RAG Retrieval
LLM
Vector Knowledge Base

Demo Video

https://drive.google.com/file/d/1sC1NNhjgYmsjP7NR6qk865IMRvv7I-bN/view?usp=drivesdk
